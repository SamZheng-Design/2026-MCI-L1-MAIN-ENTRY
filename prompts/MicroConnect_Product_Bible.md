...
**通俗理解**: 融资者是作者，把书交给图书馆上架；投资者是读者，用自己的偏好从书架上挑书、细读、借走；图书馆记录每一笔借阅，追踪每一次还书。
...
### 2.1.1 图书馆比喻（通俗理解整个系统）

> **滴灌通 = 一座"收入分成图书馆"**
>
> 融资者是**作者**，把书（项目材料）交给图书馆上架；投资者是**读者**，用自己的偏好从书架上挑书、细读、借走；图书馆记录每一笔借阅，追踪每一次还书。

| 图书馆比喻 | 对应的"通" | 动作 |
|-----------|-----------|------|
| 办借书证，选择身份（作者/读者/机构） | 身份通 | 注册 + 解锁身份 |
| 作者交稿，AI 编辑整理上架 | 发起通 | 上传材料 → 统一落库 |
| 图书馆的书架（所有书的存放处） | 数据底座 (Data) | 统一项目池 |
| 读者自建选书标准 | 评估通 | 搭建 AI 筛子 |
| 图书管理员核实书的真伪 | 风控通 | 材料验真 |
| 读者逛书架，按偏好挑书 | 参与通 | 从项目池捞项目 |
| 坐下来细读，谈借阅条件 | 条款通 | 三联动滑块磋商 |
| 签借书协议 | 合约通 | 签署合同 |
| 图书馆系统登记借阅记录 | 结算通 | 大账本记账 |
| 是否按时还书 | 履约通 | 每日回款监控 |
| 图书馆的智能系统 | AI 引擎 | GPT-4o 统一驱动 |
...
```
融资者（作者）在发起通上传经营数据（交稿）
         ↓
数据统一落库到项目池（上架到图书馆书架）
         ↓
投资者（读者）搭建的评估通筛子 → 第一轮筛选（按偏好选书）
         ↓ 通过
投资者搭建的风控通筛子 → 第二轮筛选（验真核实）
         ↓ 通过
参与通看板展示（读者看到的推荐书单）
         ↓ 投资者选中项目（挑中一本书）
条款通（谈借阅条件）→ 合约通（签借书协议）→ 结算通（登记借阅）→ 履约通（追踪还书）
```
...
**业务逻辑**:
- > 📚 图书馆比喻：**办借书证** — 走进图书馆，先办证，选择你是作者（融资者）、读者（投资者）还是机构会员，一张证多个身份随时解锁
...
**业务逻辑**:
- > 📚 图书馆比喻：**作者交稿上架** — 作者把手稿丢给图书馆，AI 编辑帮你整理成三件套（原稿归档、内容摘要、精美封面），整理好后上架到书架上
- 融资者的专属工具——融资者的"项目发起台"
...
**业务逻辑**:
- > 📚 图书馆比喻：**读者自建选书标准** — 每个读者口味不同，图书馆允许读者自己搭一套选书规则：有人只看餐饮类，有人偏好月流水 50 万以上
...
**业务逻辑**:
- > 📚 图书馆比喻：**图书管理员核实真伪** — 读者选书前，管理员先验一遍：这本书是不是正版？作者信息真实吗？ISBN 对不对得上？
...
**业务逻辑**:
- > 📚 图书馆比喻：**读者逛书架挑书** — 读者走进图书馆，看到的是经过自己选书规则过滤后的推荐书单，没设规则就看全部，挑到感兴趣的可以收藏、对比、快速翻阅摘要
- 投资者的统一项目看板——像"投资项目淘宝"
...
**业务逻辑**:
- > 📚 图书馆比喻：**坐下来细读，谈借阅条件** — 读者挑中一本书，和作者坐下来谈：借多少、借多久、租金多少
...
**业务逻辑**:
- > 📚 图书馆比喻：**签借书协议** — 条件谈妥，双方签署正式借阅协议，签完字这本书就正式"借出"了
...
**业务逻辑**:
- > 📚 图书馆比喻：**图书馆系统登记借阅记录** — 谁借了什么书、借了多少、什么条件，每一笔都清清楚楚地记在系统里
...
**业务逻辑**:
- > 📚 图书馆比喻：**是否按时还书** — 图书馆每天检查：读者有没有按时还书（回款）？如果逾期或异常，系统自动发催还通知
...
> 📚 图书馆比喻：**图书馆的书架** — 所有上架的书都统一存放在这里，这是唯一的书架，所有读者都从同一个书架上找书。

| 维度 | 说明 |
|------|------|
| **图标** | `fa-database` |
| **功能** | 数据同源 · 清洗治理 · 标准化 · 统一项目池 |
| **详细** | 统一数据中台（项目池/图书馆书架），存储所有业务数据，支持跨"通"查询。所有发起通上传的项目统一落库到这里，所有参与通的筛子都从这里捞项目。数据存储（结构化 + 非结构化）、数据查询（SQL + 全文搜索）、数据备份与恢复、数据权限控制（基于身份通的权限） |

### 4.3 AI 智能引擎

> 📚 图书馆比喻：**图书馆的智能系统** — 帮作者整理手稿、帮读者搭选书规则、帮管理员验真伪、帮双方谈条件的幕后大脑。
...
**⚠️ 铁律：设计系统中的所有 Design Token（边框、阴影、渐变、动效等）全部基于品牌色 `#5DC4B3` 定义，9 个通共享同一套 Design Token，绝不因某个通的专属色而修改任何全局 Token。** 各"通"的专属色（如身份通 `#3B82F6`、参与通 `#10B981`）**仅用于**以下 4 种场景：
1. CTA 按钮背景色和文字色
2. 功能区标题和图标色
3. 功能卡片的浅色背景底纹
4. 进度条/标签/徽章等功能性色块
...
**不得用专属色替换以下全局 Token**：
- `hover边框` 必须保持 `rgba(93, 196, 179, 0.2)` — 品牌色
- `focus边框` 必须保持 `#5DC4B3` — 品牌色
- `--shadow-card-hover` 必须保持 `rgba(93,196,179,0.1)` — 品牌色
- `--gradient-primary` 必须保持 `#5DC4B3 → #49A89A` — 品牌色
- 所有已定义的 CSS 变量（`--shadow-*`, `--ease-*`, `--gradient-*`）**原封不动照搬**
...
2. **品牌色 #5DC4B3 用于全局** — 导航栏、返回按钮、品牌相关元素、hover边框、focus边框、卡片hover阴影
3. **每个"通"的专属色仅用于内容区的 4 种场景** — CTA按钮、功能标题/图标、卡片浅色底纹、进度条/标签/徽章。**绝不替换任何全局 Design Token**
...
10. **Design Token 全局统一** — 第六章定义的所有 CSS 变量（`--shadow-*`, `--ease-*`, `--gradient-*`）、边框色、Tailwind config 在所有"通"中 100% 相同，不得因专属色而做任何修改
...
⚠️ 设计系统 100% 原封不动照搬第六/七章内容，所有 Design Token（边框、阴影、渐变）统一基于品牌色 #5DC4B3
⚠️ 该通专属色（浅色 {light}、深色 {dark}）仅限用于：CTA按钮、功能标题/图标、卡片浅色底纹、进度条/标签/徽章
⚠️ 绝不用专属色替换 hover边框/focus边框/shadow-card-hover/gradient-primary 等全局 Token
