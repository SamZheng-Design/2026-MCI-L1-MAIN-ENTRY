        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div class="text-center fade-in">
            ...
            <div class="flex justify-center">
              <div class="inline-flex bg-white rounded-xl border border-gray-200 p-1 shadow-sm">
                <button
                  class="role-btn px-5 py-2 text-sm font-semibold rounded-lg transition-all bg-[#4ECDC4] text-white"
                  data-role="all"
                ...
                ><i class="fas fa-building mr-1"></i>融资者视角</button>
                <button
                  class="role-btn px-5 py-2 text-sm font-medium rounded-lg transition-all text-gray-500 hover:text-[#4ECDC4]"
                  data-role="investor"
                  onclick="filterByRole('investor')"
                ><i class="fas fa-chart-line mr-1"></i>投资者视角</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="product-grid">
            {products.map((p) => (
              <a
                href={`/${p.id}`}
                class="portal-card block bg-white rounded-2xl p-6 no-underline product-card shadow-sm"
                data-role={p.role}
                style="border: 2px solid #E5E7EB;"
              >
                {/* Logo + Status */}
                <div class="flex justify-between items-start mb-4">
                  <ProductLogo name={p.name} englishShort={p.englishShort} size={72} />
                ...
                {/* Info */}
                <h3 class="text-lg font-bold text-black mb-0.5">{p.name}</h3>
                ...
                <div class="flex items-center gap-2 mb-4">
                  <span class="text-[10px] px-2 py-0.5 rounded-md font-semibold bg-[#4ECDC4]/10 text-[#4ECDC4] border border-[#4ECDC4]/20">
                    {p.category}
                  </span>
                  {p.role !== 'all' && (
        ...
        function filterByRole(role) {
          document.querySelectorAll('.role-btn').forEach(btn => {
            if (btn.dataset.role === role) {
              btn.className = 'role-btn px-5 py-2 text-sm font-semibold rounded-lg transition-all bg-[#4ECDC4] text-white';
            } else {
